...
union mat4 {
	...
	static mat4 inverse(const mat4& m) {
		mat4 result{};
		// Flatten to 1D array for easier formula usage: a[row*4 + col]
		float a[16];
		for (int r = 0; r < 4; ++r)
			for (int c = 0; c < 4; ++c)
				a[r * 4 + c] = m.elements[r][c];
		...
		if (fabsf(det) < 1e-9f) {
			// singular matrix: return identity (or handle error as you prefer)
		...
		// write back to result.elements[row][col]
		for (int r = 0; r < 4; ++r)
			for (int c = 0; c < 4; ++c)
				result.elements[r][c] = inv[r * 4 + c];
